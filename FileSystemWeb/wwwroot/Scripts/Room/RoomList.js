...
function sort(fsSortColumn, element) {
    if ($(element).closest('th').hasClass("sorting_asc")) {
        ...
        getRoomData(lsPageNo, fsSortColumn, "desc");
    }
    else {
        $(element).closest('th').removeClass("sorting_desc").addClass("sorting_asc");
        lsSortOrder = 'asc';
        lsSortCol = fsSortColumn;
        getRoomData(lsPageNo, fsSortColumn, "asc");
    }
}

function getRoomData(fsPageNo, fsSortColumn = lsSortCol, fsSortDirection = lsSortOrder) {
    showLoading();
    var lsRoomNumber = $('#txtRoomNumber').val();
    var liStatus = '';
    ...
    loData.RoomNumber = lsRoomNumber;
    ...
    loadMyRequest(msGetRoomDataList, "GET", loData, getRoomDataSuccess, getRoomDataError);
    $('#preloader').hide();
}
...
function getRoomDataSuccess(foResponse) {
    ...
    if (foResponse.indexOf('No records found.') > -1) {
        ...
        $('#RoomList').find("table tbody").html(foResponse);
        $('#paginationList').html('');
        ...
        $('#ddPageSize').show();
        $('#RoomList').find("table tbody").empty();
        $('#RoomList').find("table tbody").append(foResponse);
        if (lsSortCol != '') {
...
function GeneratePageListpanel(fsvalue, ffName) {
    ...
    for (liCounter = 0; liCounter <= liListPanelSize + 1; liCounter++) {
        ...
        if (liCounter == 0) {
            ...
            if (liPageNo <= liTotalpages) {
                lsHTML = lsHTML + '<li id="pagging_' + liPageNo + '" class="paginate_button page-item "><a href="javascript:onclick=getRoomData(' + liPageNo + ');" data-dt-idx="3" tabindex="0" class="page-link" id=aLinkPageNo' + liPageNo + '>' + liPageNo + '</a></li>';
            }
        }
    }
    lsHTML = lsHTML + "</ul>";
