...
function getDepartmentData(fsPageNo, fsSortColumn = lsSortCol, fsSortDirection = lsSortOrder) {
    showLoading();
    var laDepartmentName = $('#txtDepartmentName').val();
    var liStatus = '';
    ...
    loData.DepartmentName = laDepartmentName;
    ...
    loadMyRequest(msGetDepartmentDataList, "GET", loData, getDepartmentDataSuccess, getDepartmentDataError);
    $('#preloader').hide();
}

function getDepartmentDataError(foResponse) {
    hideLoading();
}

function getDepartmentDataSuccess(foResponse) {
    showLoading();
    if (foResponse.indexOf('No records found.') > -1) {
        lshasMoreRecords = false;
        $('#DepartmentList').find("table tbody").html(foResponse);
        $('#paginationList').html('');
        ...
        $('#ddPageSize').show();
        $('#DepartmentList').find("table tbody").empty();
        $('#DepartmentList').find("table tbody").append(foResponse);
        if (lsSortCol != '') {
...
function GeneratePageListpanel(fsvalue, ffName) {
    ...
    for (liCounter = 0; liCounter <= liListPanelSize + 1; liCounter++) {
        ...
        if (liCounter == 0) {
            ...
            if (liPageNo <= liTotalpages) {
                lsHTML = lsHTML + '<li id="pagging_' + liPageNo + '" class="paginate_button page-item "><a href="javascript:onclick=getDepartmentData(' + liPageNo + ');" data-dt-idx="3" tabindex="0" class="page-link" id=aLinkPageNo' + liPageNo + '>' + liPageNo + '</a></li>';
            }
        }
    }
    lsHTML = lsHTML + "</ul>";
