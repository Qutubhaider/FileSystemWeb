...
function getDesignationData(fsPageNo, fsSortColumn = lsSortCol, fsSortDirection = lsSortOrder) {
    showLoading();
    var laDesignationName = $('#txtDesignationName').val();
    var liStatus = '';
    ...
    loData.DesignationName = laDesignationName;
    ...
    loadMyRequest(msGetDesignationDataList, "GET", loData, getDesignationDataSuccess, getDesignationDataError);
    $('#preloader').hide();
}

function getDesignationDataError(foResponse) {
    hideLoading();
}

function getDesignationDataSuccess(foResponse) {
    showLoading();
    if (foResponse.indexOf('No records found.') > -1) {
        lshasMoreRecords = false;
        $('#DesignationList').find("table tbody").html(foResponse);
        $('#paginationList').html('');
        ...
        $('#ddPageSize').show();
        $('#DesignationList').find("table tbody").empty();
        $('#DesignationList').find("table tbody").append(foResponse);
        if (lsSortCol != '') {
...
function GeneratePageListpanel(fsvalue, ffName) {
    ...
    for (liCounter = 0; liCounter <= liListPanelSize + 1; liCounter++) {
        ...
        if (liCounter == 0) {
            ...
            if (liPageNo <= liTotalpages) {
                lsHTML = lsHTML + '<li id="pagging_' + liPageNo + '" class="paginate_button page-item "><a href="javascript:onclick=getDesignationData(' + liPageNo + ');" data-dt-idx="3" tabindex="0" class="page-link" id=aLinkPageNo' + liPageNo + '>' + liPageNo + '</a></li>';
            }
        }
    }
    lsHTML = lsHTML + "</ul>";
