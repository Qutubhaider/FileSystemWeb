...
function sort(fsSortColumn, element) {
    if ($(element).closest('th').hasClass("sorting_asc")) {
        ...
        getStoreData(lsPageNo, fsSortColumn, "desc");
    }
    else {
        $(element).closest('th').removeClass("sorting_desc").addClass("sorting_asc");
        lsSortOrder = 'asc';
        lsSortCol = fsSortColumn;
        getStoreData(lsPageNo, fsSortColumn, "asc");
    }
}

function getStoreData(fsPageNo, fsSortColumn = lsSortCol, fsSortDirection = lsSortOrder) {
    showLoading();
    ...
    loadMyRequest(msGetStoreDataList, "GET", loData, getStoreDataSuccess, getStoreDataError);
    $('#preloader').hide();
}

function getStoreDataError(foResponse) {
    hideLoading();
}

function getStoreDataSuccess(foResponse) {
    showLoading();
    if (foResponse.indexOf('No records found.') > -1) {
        lshasMoreRecords = false;
        $('#StoreList').find("table tbody").html(foResponse);
        $('#paginationList').html('');
        ...
        $('#ddPageSize').show();
        $('#StoreList').find("table tbody").empty();
        $('#StoreList').find("table tbody").append(foResponse);
        if (lsSortCol != '') {
